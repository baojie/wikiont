...
{
    public TermRenderer ( String onturi )
    {
        super ( onturi ) ;
    }
    ...
    /**
     *  render an atomic resource
     */
    public String renderLocalClass ( Resource res )
    {
        if ( inDomain ( res ) )
        {
            OntClass equCla = ( OntClass ) res ;

            if ( equCla.isAnon () )
            {
                return NEWLINE + renderClass ( equCla ) ;
            }
        }

        return NEWLINE + term2Link ( res ) ;
    }
    ...
    public String renderClass ( OntClass cla )
    {
        ...
        if ( ( !cla.isAnon () ) && ( cla.listEquivalentClasses () != null ) )
        {
            try
            {
                ...
                while ( iter.hasNext () )
                {
                    ...
                    equStr += renderLocalClass ( res ) ;
                }

                if ( !equStr.equals ( "" ) )
                {
                    desc += BREAKLINE + " <b>Synonyms:</b> " + equStr +
                            BREAKLINE ;
                }
            }
            catch ( Exception ex )
            {
                System.out.println ( "Error occurs when pasring " + cla.getURI () ) ;
                ex.printStackTrace () ;
            }
        }
        ...
        if ( cla.isIntersectionClass () )
        {
            ...
            {
                Resource res = ( Resource ) iter.next () ;
                intersectionStr += renderLocalClass ( res ) ;
            }
        ...
        if ( cla.isComplementClass () )
        {
            ...
            {
                Resource res = ( Resource ) iter.next () ;
                complementStr += renderLocalClass ( res ) ;
            }
        ...
        if ( ( !cla.isAnon () ) && ( cla.listSubClasses () != null ) )
        {
            ...
            {
                Resource res = ( Resource ) iter.next () ;
                subStr += renderLocalClass ( res ) ;

                // show sub-class hierarchy
                if ( inDomain ( res ) )
                {
                    OntClass subCla = ( OntClass ) res ;
        ...
        if ( ( !cla.isAnon () ) && ( cla.listSuperClasses () != null ) )
        {
            ...
            {
                Resource res = ( Resource ) iter.next () ;
                supStr += renderLocalClass ( res ) ;

                // show super-class hierarchy
                if ( inDomain ( res ) )
                {
                    OntClass subCla = ( OntClass ) res ;
                    if ( subCla.listSuperClasses () != null )
                    {
                        supStr += getSuperClassHierarchy ( subCla , 1 ) ;
                    }
                }
            }
        ...
        if ( ( !cla.isAnon () ) && ( cla.listDeclaredProperties () != null ) )
        {
            ...
            while ( iter.hasNext () )
            {
                Resource prop = ( Resource ) iter.next () ;
    ...
    public String renderRestriction ( Restriction r )
    {
        ...
        if ( r.isAllValuesFromRestriction () )
        {
            ...
            resStr += FORALL + propStr + " " + MEMBEROF + " " +
                    renderValue ( all ) ;
        }
