...
import edu.iastate.anthill.indus.owl.jena.OntModelProvider ;
...
public class JenaUtil
{
    ...
    public static OntResource asOntResource ( Resource resource )
    {
        ...
        else
        {
            ...
            OntResource res = ontModel.createOntResource (
                    OntResource.class , ontModel.getProfile ().NIL () ,
                    resource.getURI () ) ;
            if ( res == null )
            {
        ...
        if ( resource.canAs ( OntResource.class ) )
        {
            ...
            if ( type.canAs ( OntClass.class ) )
            {
                ...
                for ( Iterator it = c.listSubClasses ( true ) ; it.hasNext () ; )
                {
                    ...
                    if ( canAs ( resource ,
                                 ( OntClass ) superClass.as ( OntClass.class ) ) )
                    {
                        return true ;
                    }
                }
                                               ...
                                               OntResource ontResource )
    {
        ...
        if ( object != null )
        {
            if ( !ontModel.getBaseModel ().contains ( subject , predicate ,
                    object ) )
            {
                return true ;
            }
    ...
    public static boolean isSystemResource ( Resource ontResource )
    {
        ...
        return ( nameSpace.equals ( OWLMeta.NS ) &&
                 !OWLMeta.SLOT_VALUES_COMPUTER.equals ( ontResource.
                getLocalName () ) ) ||
    ...
    public static OntResource renameResource ( Resource old , String uri )
    {
        ...
        else
        {
            ...
            res = homeModel.createOntResource ( cls ,
                                                homeModel.getProfile ().NIL () ,
                                                uri ) ;
        }
                                               ...
                                               Resource newResource )
    {
        ...
        for ( Iterator i = m.listStatements ( old , null , ( RDFNode )null ) ;
                           i.hasNext () ; )
        {
            stmts.add ( i.next () ) ;
        }
        ...
        for ( Iterator i = stmts.iterator () ; i.hasNext () ; )
        {
            ...
            Resource subj = s.getSubject ().equals ( old ) ? newResource :
                            s.getSubject () ;
            RDFNode obj = s.getObject ().equals ( old ) ? newResource :
                          s.getObject () ;
            m.add ( subj , s.getPredicate () , obj ) ;
        }
    }

    private static OntModel refineModel ( OntModel model )
    {
        // refine model to remove all redundant statements
        // i.e. statements from RDF, RDFS, OWL schemas
        ...
        while ( sti.hasNext () )
        {
            ...
            try
            {
                ...
                if ( uri.startsWith ( RDF ) || uri.startsWith ( RDFS ) ||
                     uri.startsWith ( OWL ) )
                {
